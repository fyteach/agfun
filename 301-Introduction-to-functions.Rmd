...
![](figs/function-example-quadratic.png){width=60%}
...
1. The graph has the left endpoint  at $(-2, -2)$ and but no right endpoint. So the domain is $[-2, +\infty)$.  
1. The graph has a lowest positioned point $(-2, -2)$ but no highest positioned point. So the range is $[-2, +\infty)$.  
1. The graph is above 2 over the interval $(3, \infty)$.  
1. The graph is decreasing over the interval $(1,2)$.  
1. The graph has minima at $(-2,1)$ and $(2,1)$.  
1. The point $(x, 0)$ is on the $x$-axis. If $x=-0.5$, then it will be on the graph.
...
![](figs/function-exercise-ray-cubic.png){width=60%}
...
![](figs/function-exercise-cubic.png){width=60%}
